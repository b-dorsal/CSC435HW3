...
import retailservice.restful.view.EmployeeView;
...
public class EmployeeController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        final String getID = req.getParameter("id");
        System.out.println(getID);
        res.setContentType("application/json; charset=UTF-8");
        try (PrintWriter out = res.getWriter()) {
            try {
                Employee employee = Employee.getEmployee(getID);
                if(employee != null){
                    out.print(EmployeeView.getView(employee));
                }else{
                    out.print(EmployeeView.getView(null));
                }
